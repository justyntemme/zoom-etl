...
# Define the headers
headers = ['rrn', 'stateId', 'assetId', 'id', 'name', 'accountId', 'accountName', 'cloudType', 'regionId', 'regionName', 'service', 'resourceType', 'startDate', 'endDate', 'sheet_name']

parser = argparse.ArgumentParser()
parser.add_argument("-r", "--rows", help="number of rows to generate")
parser.add_argument("-o", "--output", help="path and filename to output to (should end in .csv for now)")
...
with open(file_output, 'w', newline='') as file:
    ...
    for _ in range(num_rows):
        start_dt = fake.date_between(start_date=dt.datetime(2020,1,1,00,00,00), end_date=dt.datetime(2024,12,1,00,00,00))
        ...
        writer.writerow([rrn, state_id, asset_id, id, name, account_id, account_name, cloud_type, region_id, region_name, service, resource_type, start_date, end_date, sheet_name])
