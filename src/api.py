...
import src.data_export as dataexport
...
def generate_kri_data_file(logger: Logger, config, start_date, end_date) -> pd.DataFrame:
    ...
    for query, query_data in config.items(): #data['config']['queries']:
        ...
        temp_frame = dataexport.query_data_to_dataframe(logger, query_data, start_date, end_date)
        temp_frame['sheet_name'] = query
        data_frame = pd.concat([data_frame, temp_frame.loc[:, ['rrn', 'stateId', 'assetId', 'id', 'name', 'accountId', 'accountName',
                                                        'cloudType', 'regionId', 'regionName', 'service', 'resourceType',
                                                        #'Passed', 
                                                        'startDate', 'endDate', 'sheet_name']]])
